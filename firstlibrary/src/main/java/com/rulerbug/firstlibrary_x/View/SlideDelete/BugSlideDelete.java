...
public class BugSlideDelete extends FrameLayout {
    ...
    public BugSlideDelete(@NonNull Context context) {
        super(context);
        init();
    }

    public BugSlideDelete(@NonNull Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();

    }

    public BugSlideDelete(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init();
    }

    public BugSlideDelete(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (!BugSlideDeleteManager.getInstence().hadOpen(BugSlideDelete.this)) {
            BugSlideDeleteManager.getInstence().closeCurrentSlideDelete();
            result = true;
        }
        return result;
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (!BugSlideDeleteManager.getInstence().hadOpen(this)) {
    ...
    private ViewDragHelper.Callback callback = new ViewDragHelper.Callback() {
        ...
        public void onViewPositionChanged(@NonNull View changedView, int left, int top, int dx, int dy) {
            ...
            if (mContentView.getLeft() == -mDeleteWidth && state == CLOSE) {
                state = OPEN;
                BugSlideDeleteManager.getInstence().regist_a_open(BugSlideDelete.this);
            }
            if (mContentView.getLeft() == 0 && state == OPEN) {
                state = CLOSE;
                BugSlideDeleteManager.getInstence().remove_a_open();
            }
    ...
    public void computeScroll() {
        if (mViewDragHelper.continueSettling(true)) {
            ViewCompat.postInvalidateOnAnimation(BugSlideDelete.this);
        }
        super.computeScroll();
    }
