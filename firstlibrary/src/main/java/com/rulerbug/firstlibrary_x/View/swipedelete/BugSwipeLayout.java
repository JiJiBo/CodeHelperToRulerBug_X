package com.rulerbug.firstlibrary_x.View.swipedelete;
...
public class BugSwipeLayout extends FrameLayout {
	...
	public BugSwipeLayout(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init();
	}

	public BugSwipeLayout(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}

	public BugSwipeLayout(Context context) {
		super(context);
		init();
	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if(!BugSwipeLayoutManager.getInstance().isShouldSwipe(this)){
			//先关闭已经打开的layout
			BugSwipeLayoutManager.getInstance().closeCurrentLayout();
	...
	private Callback callback = new Callback() {
		...
		public void onViewPositionChanged(View changedView, int left, int top,
			...
			if(contentView.getLeft()==0 && currentState!=SwipeState.Close){
				...
				BugSwipeLayoutManager.getInstance().clearCurrentLayout();
				...
				BugSwipeLayoutManager.getInstance().setSwipeLayout(BugSwipeLayout.this);
			}
		}
		@Override
	...
	public void open() {
		...
		ViewCompat.postInvalidateOnAnimation(BugSwipeLayout.this);
	}
	/**
	 * 关闭的方法
	 */
